...
def extract_idl_from_frames(vid_path, video_perc, path_video_folder, folder_path_frames, idl_filename):
    ...
    with open(idl_filename, 'w') as f:
        for i in progress(range(0,total)):
            ...
            list.append("%s/%s/fram%d.jpeg"%(path_video_folder,folder_path_frames,i))
            cv2.imwrite("%s/%s/fram%d.jpeg"%(path_video_folder,folder_path_frames,i), image[1])     # save frame as JPEG file
            image = vidcap.read()
